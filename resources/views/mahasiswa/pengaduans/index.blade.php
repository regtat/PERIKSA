<x-layout>
    ...
    <div class="flex items-center justify-between mt-3 mb-2">
            ...
            <a href="{{route('mahasiswa.pengaduans.create') }}">
                <button type="submit"
                    class="text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-black font-medium rounded-lg text-sm px-8 py-2.5 me-2 mb-2">
                    TAMBAH PENGADUAN
                </button>
            </a>
        </div>
    ...
    <!-- <div class="container mx-auto mt-1 mb-12"> -->
    <div class="container mx-auto max-w-5xl mt-2 mb-6">
                ...
                <table class="w-full text-sm text-left rtl:text-right text-black">
                    <thead class="text-xs text-gray-700 uppercase bg-EED3E6">
                        <tr class="text-center">
                            ...
                            <th scope="col" class="px-1 py-3">NIM</th>
                            <th scope="col" class="px-1 py-3">PENGADUAN</th>
                            <th scope="col" class="px-1 py-3">BUKTI FOTO</th>
                            <th scope="col" class="px-1 py-3">STATUS</th>
                            <th scope="col" class="px-1 py-3">TANGGAL</th>
                            <th scope="col" class="px-1 py-3">AKSI</th>
                        </tr>
                    </thead>
                    <tbody>
                        @php
                        ...
                        @foreach ($pengaduans as $pengaduan)
                            @if ($pengaduan->p_nim==Auth::user()->nim)
                                <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b">
                                <td scope="row" class="px-1 py-2 text-center font-medium text-gray-800 whitespace-nowrap">
                                        {{ $no++ }}
                                    </td>
                                    <td class="px-1 py-2 text-center font-medium text-gray-800">{{ $pengaduan->p_nim }}</td>
                                    <td class="px-1 py-2 font-medium text-gray-800"
                                        style="width:250px; word-wrap: break-word; word-break: break-word; hyphens:auto;">
                                        {{ $pengaduan->pengaduan }}</td>
                                    <td class="flex items-center justify-center py-2">
                                        <img src="{{ asset('/storage/pengaduan/' . $pengaduan->foto) }}" class="rounded"
                                            style="width:100px">
                                    </td>
                                    <td class="px-1 py-2 text-center font-medium text-gray-800">
                                        {{ $pengaduan->status }}</td>
                                    <td class="px-1 py-2 text-center font-medium text-gray-800">
                                        {{ \Carbon\Carbon::parse($pengaduan->tanggal)->format('Y-m-d') }}</td>
                                    <td class="px-1 py-0 flex items-center justify-center space-x-2 h-2">
                                        @if ($pengaduan->status == 'Menunggu')
                                            <a href="{{ route('mahasiswa.pengaduans.edit', $pengaduan->id) }}"
                                        ...
                                        @else
                                            <a href="{{ route('mahasiswa.pengaduans.index') }}"
                                                class="font-medium text-gray-400 cursor-not-allowed" aria-disabled="true">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px"
                                                    viewBox="0 0 24 24">
                                                    <g fill="none" stroke="#4ECB71" stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2">
                                        ...
                                        @endif
                                        @if ($pengaduan->status != 'Menunggu')
                                        <button class="font-medium text-gray-400 cursor-not-allowed" aria-disabled="true">
                                    ...
                                    <div class="relative p-4 w-full max-w-md max-h-full">
                                        <div class="relative pb-4 bg-white rounded-lg shadow dark:bg-gray-700">
                                            ...
                                            <div class="p-4 md:p-5 text-center">
                                                ...
                                                <form action="{{ route('mahasiswa.pengaduans.destroy', $pengaduan->id) }}" method="post" style="display:inline">
                                                    @csrf
                                                    @method('delete')
