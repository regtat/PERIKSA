<x-layout>
    ...
    <section class="flex items-center justify-center min-h-screen mx-auto max-w-5xl py-6">
            <div class="w-full max-w-sm z-20  bg-D9B5DD border border-gray-200 rounded-lg shadow sm:p-6 ">
                <form class="space-y-3" action="{{ route('mahasiswa.pengaduans.update', $pengaduans->id) }}"
                    method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <h5 class="text-xl font-bold text-black text-center">{{$title}}</h5>
                    <div>
                        <input type="text" name="p_nim"
                            ...
                            value="{{ old('p_nim', $pengaduans->p_nim) }}" readonly />
                    </div>

                    <div>
                        <textarea class="form-control bg-gray-50 border  text-gray-100 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 @error('pengaduan') is-invalid @enderror" name="pengaduan" rows="5">{{ old('pengaduan', $pengaduans->pengaduan) }}</textarea>
                    </div>
                    ...
                    <div>
                        <input type="date" name="tanggal"
                            class="bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5"
                            value="{{ old('tanggal', $pengaduans->tanggal) }}" />
                    </div>
